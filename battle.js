...
var attemptedHits = [];
var battleship = ['/home/khodabuccus/BattleShipProject-master/Destroyer/ShipDestroyerHull.png'];
...
// Console obj
var output = {
	"welcome": " > Bienvenue à Battlechips.  Utilisez le menu pour commencer.",
	"not": " > This option is not currently available.",
	"player1": " > Voulez vous placer vos appareils vous même ou souhaitez vous que l'ordinateur le fasse pour vous?",
	"self": " > Use the mouse and the Horizontal and Vertial buttons to place your ships on the bottom grid.",
	"overlap": " > Vous pouvez pas superposer vos appareils. Essayez encore.",
	"start": " > Utilisez la souris sur l'écran du dessus pour détruire votre adversaire. Bonne chance!",
	placed: function(name) { return " > Your " + name + " been placed."; },
	hit: function(name, type) { return " > " + name + "touché!" },
	miss: function(name) { return " > " + name + " raté!" },
	sunk: function(user, type) { return " > " + user + "'s " + type + " Coulé!" },
	lost: function(name) { return " > " + name + " a perdu sa flotte!!  Game Over." },
};
...
function gameSetup(t) {
	$(t).off() && $(".two").off();
	$(".one").addClass("self").removeClass("one").text("Placez");
	$(".multi").addClass("random").removeClass("multi").text("Au hasard");
